...
if(empty($oxpoints) || count($oxpoints)<2){
	echo "<br><br><br><br><br><center>You must select a numerical x-variable with more than 1 data point.</center>";
	die();
}
...
while($i<$steps){
	...
	$xtick=$xtick+$step;
	$bbox = imagettfbbox(12, 0, $reg, $xtick);
	$w = $bbox[4]-$bbox[0];
	imagettftext($im, 10, 0, $distanceright-$w/2, $height-30, $black, $reg, $xtick); 
...
while($b<1000){
	...
	while($i<$num){
		...
		if($ypoints[$select]==$cnames[0]){
			array_push($bootstrap1,$oxpoints[$select]);
		} else {
			array_push($bootstrap2,$oxpoints[$select]);
		}
		$i++;
	}

	$med1=Quartile($bootstrap1,0.5);
	$med2=Quartile($bootstrap2,0.5);
	if($med1=="error"||$med2=="error"){$dif=0;} 
	else {$dif=($med1-$med2)*$reverse;}
...
//draw the line for the difference between medians;
...
$w = $bbox[4]-$bbox[0];
imagettftext($im, 8, 0, $x2-$w/2, $height-75, $blue, $reg, $diff);
...
// output png and save
imagepng($im,'imagetemp/BSCIMed-'.$randomString.'.png');

echo "<img style='position:absolute;top:0px;left:0px;' src='imagetemp/BSCIMed-".$randomString.".png' />";
?>
