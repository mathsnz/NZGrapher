...
while($b<1000){
	...
	if(count($bootstrap1>0) && count($bootstrap2>0)){
		if(count($bootstrap1)>0){$mean1=format_number_significant_figures(array_sum($bootstrap1)/count($bootstrap1),5);} else {$mean1="error";}
		if(count($bootstrap2)>0){$mean2=format_number_significant_figures(array_sum($bootstrap2)/count($bootstrap2),5);} else {$mean2="error";}
		if($mean1=="error"||$mean2=="error"){$dif=0;} 
		else {$dif=($mean1-$mean2)*$reverse;}
...
// output png and save
imagepng($im,'imagetemp/BSCIMean-'.$randomString.'.png');

echo "<img style='position:absolute;top:0px;left:0px;' src='imagetemp/BSCIMean-".$randomString.".png' />";
?>
