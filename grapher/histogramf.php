...
if(empty($xpoints)){
	echo "<br><br><br><br><br><center>You must select a numerical variable for variable 1</center>";
	die();
}
if(empty($ypoints)){
	echo "<br><br><br><br><br><center>You must select a numerical variable for variable 2</center>";
	die();
}
...
foreach ($xpoints as $key => $value) {
	$i=0;
	while($i<$ypoints[$key]){
		array_push($newxpoints,$xpoints[$key]);
		$i++;
	}
}

$numcategories=count(array_unique($zpoints));
if($numcategories>5 && is_numeric($zpoints[0])==TRUE){
	$min=min($zpoints);
	$max=max($zpoints);
	$range=$max-$min;
	$i=0;
...
if($numcategories>5){
	echo "<br><br><br><br><br><center>You must select a categorical variable for variable 3 with 4 or fewer categories or a numerical variable for variable 3 which will automatically be split into 4 categories.</center>";
	die();
}
if($numcategories==0){
	$numcategories=1;
...
foreach($xpoints as $xpoint){
	...
	if(!array_key_exists($zpoint,$data)){$data[$zpoint]=array();}
	if(!array_key_exists($zpoint,$odata)){$odata[$zpoint]=array();}
	$w=0;
	while($w<$ypoints[$i]){
		array_push($data[$zpoint],strval($xbucket));
		array_push($odata[$zpoint],strval($xpoint));
		$w++;
	}
	$i++;
}
